...
class PaymentController extends Controller
{
    ...
    public function paymeCallback(Request $request)
    {
        ...
        try {
            $data = $request->all();
            $verified = $this->payme->verifyCallback($data);
            ...
            $tx = Transaction::where('provider_order_id', $orderId)->orWhere('id', $orderId)->first();
            if (!$tx) {
    ...
    public function clickCallback(Request $request)
    {
        ...
        try {
            ...
            $orderId = $data['merchant_trans_id'] ?? ($data['order_id'] ?? null);
            $status = $data['status'] ?? ($data['action'] ?? null);
            ...
            if ($status === 'completed' || $status === 'paid' || $data['action'] == 1) {
                $tx->status = 'paid';
                $tx->payload = $data;
                $tx->save();
            } else {
